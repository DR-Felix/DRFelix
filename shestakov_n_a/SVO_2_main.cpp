...
std::vector<double> collision_of_cores(Rdec2D r_1, Rdec2D v_1_x, Rdec2D v_1_y,
    ...
    double delta_t, double delta_s) {            /*delta_s - допустимая погрешность в
    ...
    while (r_1.y >= 0) {
        ...
        if (std::abs(norm(r_1 - r_2)) <= delta_s) { //столкновение
            vec.push_back(1);
            vec.push_back(t);
            return vec;
        }
...
int main() {
    ...
    if (not flag) {
        file.close();
        std::cout << "При таких параметрах сбить ядро невозможно";
    }
